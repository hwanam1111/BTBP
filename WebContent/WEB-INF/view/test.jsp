			...
			<form id="frm" action="MileStoneGoodAndBad_Insert.do">
              ...
              <c:if test="${!list.isEmpty()}"> 
               ...
               <c:if test="${MileStoneVO.checkPointArr[i] ne 0 }">
                 <c:set var="sum" value="${sum+MileStoneVO.checkPointArr[i]}" ></c:set>
               	 <c:set var="checkListNumber" value="${ checkListNumber+1}"></c:set>
                <tr>
                ...
                <tr><th>총점</th><th></th><th>${sum}점</th></tr>
				<tr><th>체크리스트 개수</th><th></th><th>${checkListNumber}개</th></tr>
				<c:set var="test" value="${sum div checkListNumber}"/>
				<fmt:formatNumber var="sumAnswer1" value="${test}" pattern="#.##"/>
				<tr><th>평균</th><th></th><th>${sumAnswer1}점</th></tr>
              </table>
            </div>
